import React, { useMemo, useState } from 'react';
import { Delete } from 'lucide-react';
import type { WeightUnit } from '../weight';
import {
  convertWeightValue,
  formatWeightInput,
  normalizeWeightValue,
  parseWeightInput,
  toWeightInputFromKg,
} from '../weight';
import { CALENDAR_FONT_STACK, WARMTRACK_FONT_STACK } from '../utils';

interface WeightPickerSheetProps {
  selectedWeightKg: number | null;
  onCancel: () => void;
  onConfirm: (weightKg: number) => void;
}

const KEYPAD_ROWS: Array<Array<{ key: string; value: string; kind: 'digit' | 'dot' | 'backspace' }>> = [
  [
    { key: '1', value: '1', kind: 'digit' },
    { key: '2', value: '2', kind: 'digit' },
    { key: '3', value: '3', kind: 'digit' },
  ],
  [
    { key: '4', value: '4', kind: 'digit' },
    { key: '5', value: '5', kind: 'digit' },
    { key: '6', value: '6', kind: 'digit' },
  ],
  [
    { key: '7', value: '7', kind: 'digit' },
    { key: '8', value: '8', kind: 'digit' },
    { key: '9', value: '9', kind: 'digit' },
  ],
  [
    { key: 'dot', value: '.', kind: 'dot' },
    { key: '0', value: '0', kind: 'digit' },
    { key: 'backspace', value: '', kind: 'backspace' },
  ],
];

export const WeightPickerSheet: React.FC<WeightPickerSheetProps> = ({ selectedWeightKg, onCancel, onConfirm }) => {
  const [unit, setUnit] = useState<WeightUnit>('kg');
  const [draftInput, setDraftInput] = useState<string>(() => toWeightInputFromKg(selectedWeightKg, 'kg'));

  const parsedDisplayValue = useMemo(() => parseWeightInput(draftInput), [draftInput]);
  const parsedWeightKg = useMemo(() => {
    if (parsedDisplayValue === null) return null;
    const converted = convertWeightValue(parsedDisplayValue, unit, 'kg');
    return normalizeWeightValue(converted);
  }, [parsedDisplayValue, unit]);
  const canConfirm = parsedWeightKg !== null && parsedWeightKg > 0;

  const displayValue = draftInput || '0';
  const unitLabel = unit === 'kg' ? '公斤' : '斤';

  const appendDigit = (digit: string) => {
    setDraftInput((current) => {
      const normalizedCurrent = current || '0';
      if (normalizedCurrent.includes('.')) {
        const decimalPart = normalizedCurrent.split('.')[1] ?? '';
        if (decimalPart.length >= 1) return normalizedCurrent;
        return `${normalizedCurrent}${digit}`;
      }
      if (normalizedCurrent === '0') return digit;
      if (normalizedCurrent.length >= 3) return normalizedCurrent;
      return `${normalizedCurrent}${digit}`;
    });
  };

  const appendDot = () => {
    setDraftInput((current) => {
      const normalizedCurrent = current || '0';
      if (normalizedCurrent.includes('.')) return normalizedCurrent;
      return `${normalizedCurrent}.`;
    });
  };

  const backspace = () => {
    setDraftInput((current) => {
      if (!current || current === '0') return '0';
      const next = current.slice(0, -1);
      if (!next || next === '.') return '0';
      return next;
    });
  };

  const switchUnit = () => {
    setUnit((currentUnit) => {
      const nextUnit: WeightUnit = currentUnit === 'kg' ? 'jin' : 'kg';
      setDraftInput((currentInput) => {
        const parsedCurrent = parseWeightInput(currentInput);
        if (parsedCurrent === null) return currentInput;
        const converted = convertWeightValue(parsedCurrent, currentUnit, nextUnit);
        return formatWeightInput(converted);
      });
      return nextUnit;
    });
  };

  const handleConfirm = () => {
    if (!canConfirm || parsedWeightKg === null) return;
    onConfirm(parsedWeightKg);
  };

  return (
    <div className="absolute inset-0 z-[70] bg-black/20">
      <button type="button" aria-label="关闭体重输入弹窗" onClick={onCancel} className="absolute inset-0" />

      <section
        className="absolute left-0 right-0 bottom-0 rounded-t-[28px] bg-[#efeff2] border-t border-rose-100 shadow-[0_-14px_34px_-28px_rgba(15,23,42,0.3)]"
        style={{ fontFamily: WARMTRACK_FONT_STACK }}
      >
        <header className="h-14 px-4 bg-white rounded-t-[28px] flex items-center">
          <button type="button" onClick={onCancel} className="text-[16px] text-slate-700 active:opacity-70">
            取消
          </button>
          <h2 className="flex-1 text-center text-[20px] font-semibold text-slate-900">体重</h2>
          <button
            type="button"
            onClick={handleConfirm}
            disabled={!canConfirm}
            className={`text-[16px] font-semibold active:opacity-70 ${canConfirm ? 'text-[#f24f8f]' : 'text-slate-300'}`}
          >
            确定
          </button>
        </header>

        <div className="px-3 pt-3 pb-2">
          <div className="h-[188px] rounded-3xl border border-slate-200 bg-white/80 px-3 py-3">
            <div className="flex justify-end">
              <button
                type="button"
                onClick={switchUnit}
                className="h-9 px-4 rounded-full border border-slate-300 bg-white text-[14px] font-medium text-slate-500 active:scale-[0.99] transition-transform"
              >
                {unit === 'kg' ? '切换为斤' : '切换为公斤'}
              </button>
            </div>
            <div className="h-[134px] -mt-5 flex items-center justify-center">
              <span
                className="text-[74px] leading-none font-medium tracking-[0.02em] tabular-nums text-slate-400"
                style={{ fontFamily: CALENDAR_FONT_STACK }}
              >
                {displayValue}
              </span>
              <span className="ml-2 text-[46px] leading-none font-medium text-slate-400">{unitLabel}</span>
            </div>
          </div>
        </div>

        <div className="px-2 pb-4">
          <div className="grid grid-cols-3 gap-3">
            {KEYPAD_ROWS.flatMap((row) =>
              row.map((key) => {
                if (key.kind === 'backspace') {
                  return (
                    <button
                      key={key.key}
                      type="button"
                      onClick={backspace}
                      className="h-[64px] rounded-2xl bg-white border border-slate-200 text-slate-600 grid place-items-center active:scale-[0.99] transition-transform"
                      aria-label="删除一位"
                    >
                      <Delete size={26} />
                    </button>
                  );
                }

                if (key.kind === 'dot') {
                  return (
                    <button
                      key={key.key}
                      type="button"
                      onClick={appendDot}
                      className="h-[64px] rounded-2xl bg-white border border-slate-200 text-[36px] leading-none font-light text-slate-700 active:scale-[0.99] transition-transform"
                    >
                      .
                    </button>
                  );
                }

                return (
                  <button
                    key={key.key}
                    type="button"
                    onClick={() => appendDigit(key.value)}
                    className="h-[64px] rounded-2xl bg-white border border-slate-200 text-[36px] leading-none font-light text-slate-700 active:scale-[0.99] transition-transform"
                  >
                    {key.value}
                  </button>
                );
              })
            )}
          </div>
        </div>
      </section>
    </div>
  );
};
